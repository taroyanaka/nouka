...
13. 横長マップの寸法、左端スポーン、右方向への敵移動、Canvasの720×360

各シナリオは`test.js`の`runScenario`から初期化され、必要なフレーム数だけ`api.step()`で更新される。敵定義13件、罠・敵能力・Wave・経路・バフ定義適用100件などをループで展開するため、ソース中のシナリオIDリテラル数と実行件数は一致しない。現在の実行予定件数は182件で、`test.js`の`expectedScenarioCount`も182である。実行時はログの`scenario_end`件数が182件であることを確認する。
...
ヘッドレスChromiumで`test.html`を読み込み、テストを自動実行した。`localStorage`の`farm-defense-test-log`に保存された結果を確認した。

- `generatedAt`: `2026-09-11T10:33:39.561Z`
- `status`: `passed`
- `totalFailures`: `0`
- `failedAssertionCount`: `0`
- `scenarioCount`: `182` / `expectedScenarioCount`: `182`
- `scenario_end`: `182`件
- ページエラー: なし

今回追加したコインプッシャー型の初期罠6種（tileの`slow`、`poison`、`burn`、`armor_down`、lineの`pull`、`knockback`）について、罠定義の件数・ID・効果種別・価格の検証も成功した。ブラウザからのログファイル保存は行わず、ページ内の実行結果を採用した。

横長レイアウト検証では、グリッドが30列×15行であること、straightマップの入口が左端・基地が右端であること、Canvasが720×360であること、敵のx座標が更新後に増加することを確認した。
...
- `scenario_end`の件数が、`test.js`内の`expectedScenarioCount`（182）と一致しない
...
- `test.js`はループで動的シナリオを生成し、実行予定件数は182件である。ソース中のシナリオIDリテラルを単純に数えると24件程度に見えるが、それは実行件数ではない。
- `test.js`の`expectedScenarioCount = 182`は、動的生成後の`scenario_end`件数を検証する値であり、現行コード上は妥当である。実際の成否は、ブラウザ実行後の`test_end`と最新ログで確認する。
...
- `ENEMY_DEFINITIONS`の全敵ID（現在13種類）

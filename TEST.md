...
各シナリオは`test.js`の`runScenario`から初期化され、必要なフレーム数だけ`api.step()`で更新される。現在の想定シナリオ数は185である。実装を変更してシナリオ数が変わった場合は、`test.js`、この文書、最新の`test-log*.json`の件数を同時に更新する。
...
- `scenario_end`の件数が想定シナリオ数185と一致しない

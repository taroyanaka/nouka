(function () {
  ...
  function runEnemyProgressionTests() {
    ...
    runScenario({scenarioId:'path_bfs_failure_after_block',phase:'path',seed:1321,setup:()=>{for(let x=0;x<15;x++)api.blockCell(x,1)},assertions:()=>assert('path_bfs_failure_after_block','path',1321,0,'path_empty',api.findPath(7,0,7,29),[])});
  ...
  function runTrapBoundaryAndValidationTests() {
    ...
    runScenario({scenarioId:'trap_knockback_pull_clamp',phase:'trap_effect',seed:1507,setup:()=>{api.spawnEnemy('peasant',0,1);api.placeTrapDefinition(make('line',{effects:[{type:'knockback',amount:99,duration:1}]}),0,1);api.spawnEnemy('peasant',2,1);api.placeTrapDefinition(make('line',{effects:[{type:'pull',amount:99,duration:1}]}),1,1,'right')},simulationFrames:1,assertions:state=>{assert('trap_knockback_pull_clamp','trap_effect',1507,1,'knockback_map_clamped',state.enemies[0].x>=2&&state.enemies[0].y>=26,true);assert('trap_knockback_pull_clamp','trap_effect',1507,1,'pull_map_clamped',state.enemies[1].x>=2&&state.enemies[1].y>=26,true);}});

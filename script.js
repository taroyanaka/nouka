...
const DEFAULT_TECH_TREES=[{id:'standard',name:'標準開拓ツリー',description:'農業と防衛をバランスよく強化',nodes:[['tech_agriculture','近代農業',1,50,[],'buff_crop_speed'],['tech_defense_1','簡易防衛線',1,50,[],'buff_mg_damage'],['tech_irrigation','市場流通改善',2,120,['tech_agriculture'],'buff_crop_sell'],['tech_ballistics','自動装填機構',2,150,['tech_defense_1'],'buff_mg_cooldown'],['tech_science_boost','学術研究推進',2,100,['tech_agriculture','tech_defense_1'],'buff_science_gen'],['tech_automation','ドローン推進器',3,250,['tech_irrigation'],'buff_drone_speed'],['tech_heavy_artillery','重火器工学',3,300,['tech_ballistics'],'buff_missile_damage,buff_missile_range'],['tech_geo_engineering','環境改変',4,450,['tech_automation','tech_heavy_artillery'],'buff_slow_tile,buff_farm_hp']]},{id:'expedition',name:'探検・発掘ツリー',description:'発掘ドローンと資源獲得を優先',nodes:[['dig_start','発掘許可',1,40,[],'buff_money_instant'],['dig_science','地質調査',1,70,[],'buff_science_gen'],['dig_speed','掘削機構',2,130,['dig_start'],'buff_drone_speed'],['dig_luck','幸運の地脈',2,180,['dig_start','dig_science'],'buff_crop_sell'],['dig_yield','資源精製',3,280,['dig_speed','dig_luck'],'buff_money_instant'],['dig_master','深層発掘',4,420,['dig_yield'],'buff_science_gen,buff_money_instant']]}];
...
function drawSprite(key,x,y,options={}){const d=spriteDefinition(key,options.definition),state=loadSprite(d.uri);if(!state?.ready)return false;const frame=Math.max(0,Math.floor((TEST_RUNTIME.frame||0)/Math.max(1,Number(d.frameDuration)||1))%Math.max(1,Math.floor(Number(d.frames)||1))),sx=Number(d.sx)+frame*Number(d.sw),sy=Number(d.sy),w=Number(d.width)||tile,h=Number(d.height)||tile;ctx.save();if(options.flip&&d.flipByDirection){ctx.translate(x,y);ctx.scale(-1,1);x=0;y=0}ctx.drawImage(state.image,sx,sy,Number(d.sw)||24,Number(d.sh)||24,x-w*Number(d.pivotX??.5),y-h*Number(d.pivotY??.5),w,h);ctx.restore();return true}
function applySavedConfig(){
 ...
 if(data.TRAP_DEFINITIONS)TRAP_DEFINITIONS=structuredClone(data.TRAP_DEFINITIONS);
...
applySavedConfig();
validateRegulations();
trapEngine=new TrapEngine({getGrid:()=>grid,getEnemies:()=>enemies,getTile:()=>tile,getCanvasWidth:()=>canvas.width,getCanvasHeight:()=>canvas.height,damageEnemy:(enemy,damage,source)=>damageEnemy(enemy,damage,source),isWaveActive:()=>inWave,drawSprite:(key,x,y,options)=>drawSprite(key,x,y,options),spawnVisualEffect:(type,x,y)=>effects.push(new SpriteEffect(x,y,type))});
function activeTree(){return TECH_TREES.find(t=>t.id===activeTreeId)||TECH_TREES[0]}function treeNodes(){return activeTree()?.nodes||[]}function node(id){return treeNodes().find(n=>n[0]===id)}function buff(type){return Object.values(buffs).reduce((s,id)=>{let b=BUFF_DEFINITIONS.find(x=>x[0]===id);return s+(b&&b[2]===type?b[3]:0)},0)}function mult(type){return 1+buff(type)}
...
function renderStats(){$('money').textContent=Math.floor(money);$('science').textContent=science;$('science-tech').textContent=science;$('farms').textContent=farms.length;$('wave').textContent=`${wave} / ${CONFIG.maxWave}`;$('drones').textContent=`🌱${drones.filter(d=>d.type==='sow').length} 💧${drones.filter(d=>d.type==='water').length} 🌾${drones.filter(d=>d.type==='harvest').length} ⛏${drones.filter(d=>d.type==='excavation').length}`}
function renderAll(){renderStats();$('build-grid').innerHTML=[['farm','🌱 畑'],['wall','🧱 壁'],['lab','🔬 研究所'],['slow','🟣 スロータイル'],['mg','🔫 マシンガン'],['missile','🚀 ミサイル'],['select','🖱 選択']].map(a=>`<button data-build="${a[0]}">${a[1]}<span class="cost">${a[0]==='select'?'':'$'+getBuildingCost(a[0])}</span></button>`).join('');$('drone-shop').innerHTML=[['sow','🌱 種まき'],['water','💧 水撒き'],['harvest','🌾 刈取り'],['excavation','⛏ 発掘']].map(([t,n])=>`<button class="drone-btn" data-drone="${t}">${n}<span class="cost">$${getDroneCost(t)}</span></button>`).join('');document.querySelectorAll('[data-build]').forEach(b=>b.onclick=()=>buildMode=b.dataset.build);document.querySelectorAll('[data-drone]').forEach(b=>b.onclick=()=>buyDrone(b.dataset.drone));$('buff-list').innerHTML=Object.values(buffs).length?Object.values(buffs).map(id=>`<span class="badge">${BUFF_DEFINITIONS.find(x=>x[0]===id)?.[1]||id}</span>`).join(''):'まだバフはありません';renderTech();renderTreeEditor();renderParams();renderTreeChoices()}
function buyDrone(t){let c=Math.floor(CONFIG.costDrone*Math.pow(1.5,drones.filter(d=>d.type===t).length));if(money<c){toast('資金が足りません');return}money-=c;drones.push(new Drone(t));renderAll()}
...
setTimeout(()=>{const host=document.getElementById('tech-editor');if(!host)return;const setTierRange=()=>host.querySelectorAll('[data-node-field="tier"]').forEach(input=>{input.max='99';input.placeholder='TIER'});new MutationObserver(setTierRange).observe(host,{childList:true,subtree:true});setTierRange()},0);
...
function randomBlockedCells(){
  ...
  const candidates=[];
  for(let y=1;y<CONFIG.gridRows-1;y++)for(let x=0;x<CONFIG.gridCols;x++){
    const key=`${x},${y}`;
...
function renderMapSelector(){let host=$('map-selector');if(!host){host=document.createElement('div');host.id='map-selector';$('sidebar')?.insertBefore(host,$('sidebar').children[1]||null)}host.innerHTML=`<label class="map-selector-label">マップ <select id="map-select">${Object.entries(MAP_DEFINITIONS).map(([id,m])=>`<option value="${id}" ${id===activeMapId?'selected':''}>${m.name}</option>`).join('')}</select></label>`;host.querySelector('#map-select').onchange=e=>{activeMapId=e.target.value;activeMap=MAP_DEFINITIONS[activeMapId];resetGame()}}
...
validPlace=function(x,y){if(x<0||y<1||x>=CONFIG.gridCols||y>=CONFIG.gridRows-1||!isMapCell(x,y)||grid[y]?.[x])return false;return canMapReachBase(x,y)};
configureActiveMap();
function renderParams(){
 ...
 document.querySelectorAll('[data-buff-effect]').forEach(i=>i.oninput=()=>{let b=BUFF_DEFINITIONS.find(x=>x[0]===i.dataset.buffEffect);b[3]=Number(i.value);i.previousElementSibling.querySelector('output').value=i.value});
 document.querySelectorAll('[data-buff-rarity]').forEach(i=>i.oninput=()=>{let b=BUFF_DEFINITIONS.find(x=>x[0]===i.dataset.buffRarity);b[5]=Math.max(1,Math.floor(Number(i.value)||1));i.previousElementSibling.querySelector('output').value=`R${b[5]}`});
...
const extDef = id => BUFF_DEFINITIONS.find(b=>b[0]===id);
...
place=function(x,y,type){const before=money,key={farm:'costFarm',wall:'costWall',lab:'costLab',slow:'costSlow',mg:'costMG',missile:'costMissile'}[type],old=key?CONFIG[key]:undefined;if(key)CONFIG[key]=old*extDiscount('building');const ok=originalPlace(x,y,type);if(key)CONFIG[key]=old;if(ok)emitBuffEvent('buildingPlaced',{type,cost:before-money});return ok};
const originalBuyDrone=buyDrone;
buyDrone=function(type){const base=CONFIG.costDrone*Math.pow(1.5,drones.filter(d=>d.type===type).length),required=Math.floor(base*extDiscount('drone'));if(money<required){toast('資金が足りません');return}const old=CONFIG.costDrone;CONFIG.costDrone=required/Math.pow(1.5,drones.filter(d=>d.type===type).length);originalBuyDrone(type);CONFIG.costDrone=old};
...
renderDifficultyEditor=function(){
 ...
 host.querySelectorAll('[data-regulation-tab]').forEach(b=>b.onclick=()=>{const current=activeRegulation();if(current&&TECH_TREES?.length)current.techTrees=structuredClone(TECH_TREES);activeRegulationId=b.dataset.regulationTab;currentDifficultyLevel=1;resetGame()});
 host.querySelector('#add-regulation').onclick=()=>{const source=activeRegulation(),id=`regulation_${Date.now()}`;REGULATIONS.push({id,name:`新しいレギュレーション`,difficulties:structuredClone(source.difficulties),techTrees:structuredClone(source.techTrees)});activeRegulationId=id;currentDifficultyLevel=1;resetGame()};
...
draw=function(){
 ...
 farms.forEach(f=>{if(!drawSprite('farm',f.gx*tile+tile/2,f.gy*tile+tile/2)){ctx.fillStyle=colors.farm;ctx.fillRect(f.gx*tile+2,f.gy*tile+2,tile-4,tile-4)}ctx.fillStyle='#19352a';ctx.fillRect(f.gx*tile+3,f.gy*tile+tile-5,(tile-6)*Math.max(0,f.hp/f.maxHp),2)});
 ...
 enemies.forEach(e=>{if(!drawSprite('enemy',e.x,e.y,{definition:e.definition,flip:e.moveX<0})){ctx.fillStyle=e.definition?.color||'#e85d5d';ctx.beginPath();ctx.arc(e.x,e.y,8,0,Math.PI*2);ctx.fill()}ctx.fillStyle='#fff';ctx.font='9px sans-serif';ctx.textAlign='center';ctx.fillText(e.definition?.nameJa||e.type,e.x,e.y-11)});
 drones.forEach(d=>{if(!drawSprite('drone',d.x,d.y)){ctx.fillStyle='#f8c85d';ctx.beginPath();ctx.arc(d.x,d.y,4,0,Math.PI*2);ctx.fill()}});bullets.forEach(b=>{if(!drawSprite(b.t?.type==='missile'?'missile':'bullet',b.x,b.y)){ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(b.x,b.y,2,0,Math.PI*2);ctx.fill()}});effects.forEach(e=>{if(e instanceof Explosion){if(!drawSprite('explosion',e.x,e.y)){ctx.strokeStyle='#ffb347';ctx.beginPath();ctx.arc(e.x,e.y,Math.max(2,20-e.life),0,Math.PI*2);ctx.stroke()}}else if(e instanceof SpriteEffect){if(!drawSprite('trapEffect',e.x,e.y,{definition:{}})){ctx.fillStyle='#d48cff';ctx.globalAlpha=Math.max(0,e.life/24);ctx.fillRect(e.x-5,e.y-5,10,10);ctx.globalAlpha=1}}else if(e.t){ctx.fillStyle=e.c;ctx.fillText(e.t,e.x,e.y)}});trapEngine.draw(ctx);
};
...
function setDifficultyLevel(value){
 const next=Math.max(1,Math.min(5,Math.floor(Number(value)||1)));
...
function testTryPlaceTrapDefinition(definition,x,y){const result=trapEngine.place(Number(x),Number(y),structuredClone(definition));if(result.ok)inWave=true;return {ok:result.ok,error:result.error||null}}
function testSetWaveActive(value){inWave=Boolean(value);spawning=false;return testState()}
function testSetDifficultyConfig(level,config){const n=Math.max(1,Math.min(5,Math.floor(Number(level)||1)));activeRegulation().difficulties[n]=structuredClone(config);currentDifficultyLevel=n;return structuredClone(activeRegulation().difficulties[n])}
...
function testApplyBuff(id){const definition=BUFF_DEFINITIONS.find(b=>b[0]===id);if(!definition)throw new Error(`Unknown buff: ${id}`);if(typeof addExtendedBuff==='function'&&EXTENDED_BUFFS.defs.some(b=>b[0]===id))addExtendedBuff(id);else{buffs[id]=id;if(definition[2]==='instant_money')money+=Number(definition[3])||0}return id}

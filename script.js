...
const SPRITE_DEFAULT={uri:'',sx:1,sy:1,sw:24,sh:24,width:24,height:24,pivotX:.5,pivotY:.5,frames:1,columns:0,frameDuration:8,flipByDirection:false,fallback:'shape'};
const SPRITE_CONFIG={outside:{...SPRITE_DEFAULT},floor:{...SPRITE_DEFAULT},blocked:{...SPRITE_DEFAULT},slow:{...SPRITE_DEFAULT},spawn:{...SPRITE_DEFAULT},base:{...SPRITE_DEFAULT},excavation:{...SPRITE_DEFAULT},farm:{...SPRITE_DEFAULT},wall:{...SPRITE_DEFAULT},lab:{...SPRITE_DEFAULT},mg:{...SPRITE_DEFAULT},missile:{...SPRITE_DEFAULT},trap:{...SPRITE_DEFAULT},enemy:{...SPRITE_DEFAULT},enemy_peasant:{...SPRITE_DEFAULT},enemy_adventurer:{...SPRITE_DEFAULT},enemy_warrior:{...SPRITE_DEFAULT},enemy_thief:{...SPRITE_DEFAULT},enemy_horseman:{...SPRITE_DEFAULT},enemy_knight:{...SPRITE_DEFAULT},enemy_brute:{...SPRITE_DEFAULT},enemy_showcase_titan:{...SPRITE_DEFAULT},enemy_priest:{...SPRITE_DEFAULT},enemy_drummer:{...SPRITE_DEFAULT},enemy_shieldmaster:{...SPRITE_DEFAULT},enemy_wizard_speed:{...SPRITE_DEFAULT},enemy_wizard_attack:{...SPRITE_DEFAULT},enemy_wizard_heal:{...SPRITE_DEFAULT},drone:{...SPRITE_DEFAULT},droneSow:{...SPRITE_DEFAULT},droneWater:{...SPRITE_DEFAULT},droneHarvest:{...SPRITE_DEFAULT},droneExcavation:{...SPRITE_DEFAULT},bullet:{...SPRITE_DEFAULT},bulletMG:{...SPRITE_DEFAULT},bulletMissile:{...SPRITE_DEFAULT},explosion:{...SPRITE_DEFAULT},trapEffect:{...SPRITE_DEFAULT},sowEffect:{...SPRITE_DEFAULT},waterEffect:{...SPRITE_DEFAULT},harvestEffect:{...SPRITE_DEFAULT},excavationEffect:{...SPRITE_DEFAULT}};
...
function drawSprite(key,x,y,options={}){const d=spriteDefinition(key,options.definition),state=loadSprite(d.uri);if(!state?.ready)return false;const frameCount=Math.max(1,Math.floor(Number(d.frames)||1)),frame=Math.max(0,Math.floor((TEST_RUNTIME.frame||0)/Math.max(1,Number(d.frameDuration)||1))%frameCount),columns=Math.max(1,Math.floor(Number(d.columns)||frameCount)),sx=Math.max(0,Number(d.sx||1)-1)+(frame%columns)*Number(d.sw),sy=Math.max(0,Number(d.sy||1)-1)+Math.floor(frame/columns)*Number(d.sh),w=Number(d.width)||tile,h=Number(d.height)||tile;ctx.save();ctx.translate(x,y);if(Number.isFinite(Number(options.rotation)))ctx.rotate(Number(options.rotation));if(options.flip&&d.flipByDirection)ctx.scale(-1,1);ctx.drawImage(state.image,sx,sy,Number(d.sw)||24,Number(d.sh)||24,-w*Number(d.pivotX??.5),-h*Number(d.pivotY??.5),w,h);ctx.restore();return true}
...
draw=function(){
 ...
 enemies.forEach(e=>{const enemyKey=enemySpriteKey(e.type);if(!drawSprite(enemyKey,e.x,e.y,{definition:e.definition,flip:e.moveX<0})&&!drawSprite('enemy',e.x,e.y,{definition:e.definition,flip:e.moveX<0})){ctx.fillStyle=e.definition?.color||'#e85d5d';ctx.beginPath();ctx.arc(e.x,e.y,8,0,Math.PI*2);ctx.fill()}});

...
renderDifficultyEditor=function(){
 ...
 host.querySelectorAll('[data-regulation-tab]').forEach(b=>b.onclick=()=>{const current=activeRegulation();if(current&&TECH_TREES?.length)current.techTrees=structuredClone(TECH_TREES);activeRegulationId=b.dataset.regulationTab;currentDifficultyLevel=1;resetGame({persistTechTrees:false})});
